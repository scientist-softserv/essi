...
RSpec.describe IuMetadata::AttributeIngester do
  ...
  let(:foo_context) {
    { "@context" => {
      "id" => "@id",
      "foo" => { "@id" => "http://library.princeton.edu/terms/source_metadata" }
    } }
  }
  ...
  describe "#raw_attributes" do
    ...
    context "with a mapping and destination" do
      context "with a different name" do
        ...
        it "passes the value to the new attribute" do
          expect(att_ingester.raw_attributes['source_metadata']).to eq 'bar'
        end
      end
      context "with a single-valued attribute" do
        context "with a single value" do
          let(:attributes) { { source_metadata: 'foobar' } }

          it "passes the attribute" do
            expect(att_ingester.raw_attributes['source_metadata']).to eq 'foobar'
          end
        end
        context "with multiple values" do
          let(:attributes) { { source_metadata: ['foo', 'bar'] } }
